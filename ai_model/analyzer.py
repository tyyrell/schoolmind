"""
SchoolMind AI v18 — Nour Ultimate Intelligence Engine
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
النسخة النهائية القصوى:
✦ 6000+ عبارة وكلمة مفتاحية
✦ 8 لغات/لهجات: فصحى، أردني، شامي، مصري، خليجي، مغربي، إنجليزي، فرنكو
✦ كشف التكثيف العاطفي (Intensity Amplifiers)
✦ كشف السياق المتناقض (Contradictory Context)
✦ كشف استخدام المجاز والكناية عن الألم
✦ تحليل الأنماط الزمنية (Temporal Pattern)
✦ مقياس الإلحاح (Urgency Scale 1-5)
✦ كشف تعدد المشكلات (Co-occurrence Detection)
✦ درجة الثقة بالتحليل محسّنة بـ Bayesian estimation
✦ 700+ رد لنور مع ذاكرة سياقية كاملة
✦ ردود متخصصة لكل سيناريو (12 سيناريو)
✦ كشف الأنماط المتكررة عبر المحادثة
"""
import re
import random
import logging
...
def analyze_text(text: str) -> dict:
    """Comprehensive text analysis with 18 dimensions."""
...
def analyze_survey(answers: dict) -> dict:
    map10 = {
        'q1': ('depression', 1.2, True),
        'q2': ('bullying',   1.5, False),
        'q3': ('anxiety',    1.1, False),
        'q4': ('sleep',      0.8, True),
        'q5': ('bullying',   2.0, True),
        'q6': ('isolation',  1.2, False),
        'q7': ('self_worth', 1.0, True),
        'q8': ('academic',   0.9, False),
        'q9': ('anger',      1.0, False),
        'q10':('depression', 1.3, True),
    }
    cs = defaultdict(float); tw = 0.0; tmax = 0.0
    for q,(cat,w,inv) in map10.items():
        try: v = max(1,min(5,int(answers.get(q,3) or 3)))
        except: v = 3
        rv = (6-v) if inv else v
        cs[cat] += rv*w; tw += rv*w; tmax += 5*w
    norm = round(tw/max(tmax,1)*10, 2)
    dom  = max(cs, key=cs.get) if cs else 'neutral'
    if norm < 2.0: dom = 'neutral'
    level = ('critical' if norm>=7.5 else 'high' if norm>=5.0 else 'medium' if norm>=2.5 else 'low')
    CAR = {'bullying':'تنمر أو عدم الأمان','depression':'حزن عميق أو انخفاض المزاج',
           ...
           'self_worth':'انخفاض الثقة بالنفس','academic':'ضغط أكاديمي','neutral':'حالة جيدة'}
    CEN = {'bullying':'Bullying or feeling unsafe','depression':'Deep sadness',
           ...
           'self_worth':'Low self-confidence','academic':'Academic pressure','neutral':'Generally good'}
    IC  = {'bullying':'🛡️','depression':'🌧️','anxiety':'😰','isolation':'💙',
           'anger':'😤','sleep':'😴','self_worth':'💪','academic':'📚','neutral':'😊'}
    ...
    return {'score':norm,'level':level,'emoji':em.get(level,'🟢'),'color':cl.get(level,'#10b981'),
            ...
            'category_scores':{k:round(v,2) for k,v in cs.items()},
            'breakdown':{q:answers.get(q,3) for q in map10},
            'should_alert':norm>=5.0,'max_score':10}

# ══════════════════════════════════════════════════════════════════════════════
# SECTION 6 — NOUR AI RESPONSE ENGINE (700+ responses)
# ══════════════════════════════════════════════════════════════════════════════
...
def get_companion_response(emotion: str, lang: str = 'ar', text: str = '',
    ...
    seed = abs(hash((text[:30] if text else '') + emotion)) % max(len(avail), 1)
