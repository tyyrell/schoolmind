    ...
    try:
        if "user_id" in session:
            ...
            if user and user["is_active"]:
                ...
                age_category = _age_category(user["grade"] if "grade" in user.keys() else "")
            else:
...
def _age_category(grade):
    """Map a grade string (AR or EN) to one of: age-kids, age-teens, age-seniors.
    Returns empty string if grade unknown — the UI then falls back gracefully."""
    if not grade:
        return ""
    ...
    m = _re.search(r"\d+", g)
    if not m:
        # Try Arabic grade names
        if any(k in g for k in ["رابع", "خامس", "سادس", "سابع"]):
            return "age-kids"
        if any(k in g for k in ["ثامن", "تاسع", "عاشر"]):
            return "age-teens"
        if any(k in g for k in ["حادي عشر", "ثاني عشر"]):
            return "age-seniors"
        return ""
    n = int(m.group())
    if n <= 7:   return "age-kids"
    if n <= 10:  return "age-teens"
    return "age-seniors"
...
def update_alert(aid):
    ...
    # Keep backward compat with old statuses (reviewed, handled)
    valid = ("new", "seen", "contacted", "resolved", "reviewed", "handled")
...
@app.route("/counselor/alerts")
@counselor_required
def counselor_alerts():
    """Dedicated alerts page with full workflow UI."""
    sg = _supervised_grades()
    alerts = get_alerts_for_counselor(sg or None, limit=200)
    # Fetch any locations for these students (counselor-only privacy)
    from database import get_student_location
    rows = []
    for a in alerts:
        a = dict(a)
        try:
            loc = get_student_location(a["user_id"])
            a["latitude"]  = loc["latitude"]  if loc else None
            a["longitude"] = loc["longitude"] if loc else None
        except Exception:
            a["latitude"] = a["longitude"] = None
        rows.append(a)
    status_filter = request.args.get("status", "")
    if status_filter in ("new", "seen", "contacted", "resolved"):
        rows = [r for r in rows if r.get("status") == status_filter]
    return render_template("counselor_alerts.html",
                           alerts=rows,
...
def api_companion():
    ...
    # EMERGENCY ESCALATION: if crisis keywords detected in the message,
    # create an alert for counselors and flag the response.
    is_crisis = _check_crisis(msg)
    if is_crisis:
        try:
            u = get_user_by_id(uid)
            save_alert(
                uid,
                u["anonymous_code"] or "S-????",
                u["grade"] or "",
                {"score": 9.0, "level": "critical", "max_score": 10},
                {
                    "dominant_emotion": emotion,
                    "has_bullying": False,
                    "summary": "Crisis keywords detected in Nour chat",
                    "source": "nour_crisis_filter",
                },
                lang,
            )
            log.warning(f"audit: crisis_alert_created uid={uid} source=nour_chat")
        except Exception as e:
            log.error(f"Failed to save crisis alert: {e}")

    return jsonify({
        "response": response,
        "emotion": emotion,
        "emergency": bool(is_crisis),
    })
